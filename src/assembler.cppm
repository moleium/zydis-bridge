...
export namespace zydis::assembler {
  ...
  [[nodiscard]] constexpr mem ptr(registers base, ZyanI64 disp = 0, ZyanU16 size_override = 0) {
    return mem{
      .base = static_cast<ZydisRegister>(base), .disp = disp, .size_override = size_override
    };
  }
    ...
    encode(ZyanU64 runtime_address = ZYDIS_RUNTIME_ADDRESS_NONE) const {
      ...
      if (!ZYAN_SUCCESS(status)) {
        throw std::runtime_error(
          std::string("failed to encode instruction: ") + ZydisMnemonicGetString(m_mnemonic)
        );
      }
      ...
      switch (op.type) {
        ...
        default:
          return std::unexpected(
            std::string("unsupported operand type for conversion: ") + std::to_string(op.type)
          );
      }
    }
  } // namespace detail
...
public:
    ...
    from_bytes(const std::uint8_t* data, std::size_t size) {
      ...
      while (offset < size) {
        ...
        if (!decoded_opt) {
          return std::unexpected("failed to disassemble at offset: " + std::to_string(offset));
        }
