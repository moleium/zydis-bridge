...
export namespace zydis {
  ...
  std::optional<std::vector<formatted_token>> tokenize(
    const instruction& target_instruction, ZyanU64 runtime_address = ZYDIS_RUNTIME_ADDRESS_NONE
  ) {
    ...
    ZyanStatus status = ZydisFormatterTokenizeInstruction(
      &formatter, &target_instruction.decoded, target_instruction.operands.data(),
      target_instruction.operands.size(), token_buffer.data(), token_buffer.size(), runtime_address,
      &token, nullptr
    );
